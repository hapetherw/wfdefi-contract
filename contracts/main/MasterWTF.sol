...
contract MasterWTF is IMasterWTF, CoreRef, ReentrancyGuard {
    ...
    function updateStake(uint256 _pid, address _account, uint256 _amount)
        public
        override
    ...
    function next(uint256 _cid) public override onlyRole(MASTER_ROLE) nonReentrant {
        ...
        cycleId = _cid;
        for (uint256 i = 0; i < poolInfo.length; i++) {
            poolSnapshot[cycleId][i] = PoolStatus({
                totalSupply: 0,
                lastRewardBlock: 0,
                accRewardPerShare: 0
            });
    ...
    function claim(uint256 _pid) public override nonReentrant {
        uint256 pending;
    ...
    function claimAll() public override nonReentrant {
        uint256 pending = 0;
        for (uint256 i = 0; i < poolInfo.length; i++) {
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal returns (uint256) {
        ...
        require(
            ...
            "safeRewardTransfer: Transfer failed"
        );
        return amount;
    }
