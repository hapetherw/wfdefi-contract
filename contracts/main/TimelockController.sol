...
contract TimelockController is CoreRef, ReentrancyGuard {
    ...
    function stop(address _trancheMaster) public onlySelf {
        ITrancheMaster(_trancheMaster).stop();
    }

    // IStrategyToken

    function changeRatio(address _token, uint256 _index, uint256 _value) public onlySelf {
    ...
    function inCaseTokensGetStuck(
        ...
        IStrategy(_strategy).inCaseTokensGetStuck(
            _token,
            _amount,
            _to
        );
    ...
    function setEarnedToWantPath(
        address _strategy,
        address[] memory newPath
    ) public onlyRole(EXECUTOR_ROLE) {
        IStrategy(_strategy).setEarnedToWantPath(newPath);
    }
